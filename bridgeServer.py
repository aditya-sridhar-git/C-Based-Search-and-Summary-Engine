...
OLLAMA_MODEL = "phi"  # Better quality model
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _serve_html(self):
        ...
        try:
            ...
            if html_path:
                ...
                self.wfile.write(
                    b"<h1>HTML file not found. Please place index.html in the same directory.</h1>"
                )
        except Exception as e:
    ...
    def _perform_search(self):
        ...
        try:
            ...
            type_names = {
                ...
                "multi": "Multi-Keyword AND Search",
            }
            ...
            if search_type == "keyword":
                ...
                log_data_structure(
                    "LINKED_LIST", "Walking document list to count occurrences"
                )
            ...
            log_result(
                ...
                results.get(
                    "total_matches",
                    results.get("total_occurrences", len(results.get("results", []))),
                ),
            )
            for r in results.get("results", [])[:5]:
                if "docName" in r:
                    print(
                        f"      - {r.get('docName')}: {r.get('frequency', r.get('score', 0))} hits"
                    )
                elif "word" in r:
    ...
    def _handle_autocomplete(self):
        ...
        try:
            ...
            log_data_structure(
                "TRIE", f"Walking path: " + " -> ".join(list(normalized_query))
            )
            ...
            if suggestions:
                print(
                    f"      Suggestions: {', '.join(suggestions[:7])}{'...' if len(suggestions) > 7 else ''}"
                )
            print("=" * 60 + "\n")
            # ===== END LOGGING =====
    ...
    def _handle_analyze(self):
        ...
        try:
            ...
            print(f"  Query: '{query}'")
            print(
                f"  Document size: {len(content)} chars, {len(content.split())} words"
            )
            ...
            if action == "freq":
                ...
                log_data_structure(
                    "TRIE", f"Traversing path: root", " -> ".join(list(normalized))
                )
            ...
            if not os.path.exists(cli_path):
                raise ValueError(
                    "C search engine not compiled. Run: gcc searchCLI.c -o searchCLI.exe"
                )
            ...
            if action == "freq":
                if c_result.get("found"):
                    ...
                    for doc in docs[:3]:  # Show first 3
                        print(
                            f"          - {doc.get('filename')}: {doc.get('frequency')} occurrences"
                        )
                else:
            ...
            self._set_headers(500)
            self.wfile.write(
                json.dumps({"error": f"Invalid C output: {str(e)}"}).encode()
            )
    ...
    def _handle_topk(self):
        ...
        try:
            ...
            log_data_structure(
                "HASH_TABLE", "Inserting word->trie_node mappings for O(1) lookup"
            )
            ...
            log_data_structure(
                "ARRAY",
                f"QuickSort O(n log n) on {len(set(content.lower().split()))} unique words",
            )
            ...
            for i, word_data in enumerate(c_result.get("top_words", []), 1):
                print(
                    f"      {i}. '{word_data.get('word')}' - frequency: {word_data.get('frequency')}"
                )
            print("=" * 60 + "\n")
            # ===== END LOG RESULTS =====
    ...
    def _get_chats(self):
        ...
        try:
            ...
            log_data_structure(
                "SPLAY_TREE", "Left subtree -> Node -> Right subtree (BST property)"
            )

            log_step(3, "Sorting results by timestamp")
            log_data_structure(
                "ARRAY", "QuickSort on collected nodes by timestamp (descending)"
            )
            ...
            if not os.path.exists(splay_path):
                ...
                self._set_headers()
                self.wfile.write(
                    json.dumps({"success": True, "count": 0, "chats": []}).encode()
                )
                return
            ...
            if chats:
                ...
                for chat in chats[:5]:  # Show first 5
                    print(
                        f"      - ID: {chat.get('id')}, Title: {chat.get('title', 'Untitled')[:30]}"
                    )
                if len(chats) > 5:
    ...
    def _add_chat(self):
        ...
        try:
            ...
            log_data_structure(
                "SPLAY_TREE", f"Comparing '{chat_id}' with existing nodes"
            )
            log_data_structure(
                "SPLAY_TREE", "Traversing: if id < node.id -> go left, else -> go right"
            )

            log_step(3, "Creating new node at leaf position")
            log_data_structure(
                "SPLAY_TREE",
                "Allocating ChatNode with id, title, timestamp, parent ptr",
            )
            ...
            log_data_structure(
                "SPLAY_TREE",
                "Recently accessed nodes bubble to top -> O(log n) amortized",
            )
    ...
    def _access_chat(self):
        ...
        try:
            ...
            log_data_structure(
                "SPLAY_TREE", "Moving accessed node to ROOT via rotations"
            )
    ...
    def _clear_chats(self):
        ...
        try:
            ...
            log_data_structure(
                "SPLAY_TREE", "Post-order traversal: delete children before parent"
            )
            ...
            log_data_structure(
                "SPLAY_TREE", "Overwriting chat_history.json with empty state"
            )
            ...
            if not os.path.exists(splay_path):
                ...
                self.wfile.write(
                    json.dumps(
                        {"success": True, "message": "No splay tree to clear"}
                    ).encode()
                )
                return
